...
export default function ClassroomMode() {
  ...
  return <div className="container" style={{ paddingTop:"1rem", paddingBottom:"2rem" }}>
    <div className="kc-card" style={{ marginBottom:"0.75rem" }}><div className="section-title">Classroom Mode <span style={{fontSize:"0.75rem",color:"#94a3b8"}}>Prototype • Local demo • No real multiplayer yet</span></div>
    ...
    {tab==="teacher" && <div className="kc-card"><div className="section-title">Teacher Mode</div>
      <div style={{display:"grid",gap:"0.45rem"}}>
        ...
        <div style={{display:"flex",gap:"0.4rem",flexWrap:"wrap"}}><button className="btn-secondary" onClick={()=>setSession({...session,code:generateCode()})}>Generate code</button><button className="btn-gold" onClick={()=>setSession({...session,status:"active"})}>Start session</button><button className="btn-danger" onClick={()=>setSession({...session,status:"ended"})}>End session</button></div>
      </div>
    ...
    {tab==="student" && <div className="kc-card"><div className="section-title">Student Join (Local demo)</div>
      {!joined ? <div style={{display:"grid",gap:"0.45rem",maxWidth:420}}><input className="kc-input" value={studentCode} onChange={e=>setStudentCode(e.target.value)} placeholder="Game code"/><input className="kc-input" value={studentName} onChange={e=>setStudentName(e.target.value)} placeholder="Player name"/><button className="btn-gold" onClick={()=>setJoined(true)}>Join</button></div> : <div style={{color:"#94a3b8"}}>Hi {studentName || "طالب"} — waiting for teacher to start. Ready ✅</div>}
    </div>}
    ...
    {tab==="ai" && <div className="kc-card"><div className="section-title">AI Helper (Prototype)</div>
      ...
      <div style={{display:"grid",gap:"0.45rem",maxWidth:520}}>
        ...
        <div style={{display:"flex",gap:"0.45rem",flexWrap:"wrap"}}>{["Generate Questions","Improve Question Wording","Suggest Answer Choices","Suggest Distractors","Translate Questions","Create Arabic Letters Quiz","Create Islamic Values Quiz","Create Vocabulary Quiz"].map(a=><button key={a} className="btn-secondary" onClick={()=>setAiOut(generateMockQuestions(aiReq))}>{a}</button>)}</div>
      </div>
