          ...
          >
            {rowCells.map((cell) => {
              ...
              if (isWinning) {
                ...
                shadow = "0 0 0 2px rgba(251,191,36,0.35), 0 0 20px rgba(251,191,36,0.45)";
              }

              const clickable =
                !!onCellClick &&
                (mode === "setup" || (mode === "host-game" && cell.claimedBy === 0 && !cell.used));
              ...
              return (
                <button
                  ...
                  title={
                    mode === "setup"
                      ? hasQ
                        ? "تم إعداد سؤال — اضغط للتعديل"
                        : "لا يوجد سؤال بعد — اضغط للإضافة"
                      : cell.claimedBy !== 0
                        ? "تم حجز هذا الحرف"
                        : ""
                  }
                  aria-label={`خلية ${cell.label}${claimed1 ? ` للفريق ${team1.name}` : claimed2 ? ` للفريق ${team2.name}` : ""}`}
                  style={{
                    width: cellSize,
                    height: cellSize,
                    clipPath: HEX_CLIP,
                    ...
                    transition: motionReduced ? "none" : "all 0.2s ease",
                    boxShadow: shadow,
                    outline: border,
                    outlineOffset: "-2px",
                    animation: isSelected && !motionReduced ? "hexGlow 1.5s ease-in-out infinite" : "none",
                    userSelect: "none",
                    borderWidth: 0,
                    padding: 0,
                  }}
                >
                  ...
                  {(claimed1 || claimed2) && (
                    <span style={{ fontSize: cellSize * 0.22, color: "rgba(255,255,255,0.9)", lineHeight: 1, fontWeight: 800 }}>
                      {claimed1 ? "◆" : "●"}
                    </span>
                  )}
                </button>
